...
namespace ctranslate2 {
  ...
  BiasedDecoder::BiasedDecoder() {
    _spare_float_beam = StorageView(DataType::FLOAT);
  }
  ...
  void BiasedDecoder::decode(
    ...
    if (_spare_float_beam.device() != device) {
      _spare_float_beam = StorageView(device, DataType::FLOAT);
    }
    ...
    for (dim_t b = 0; b < num_beams; ++b) {
      ...
      if (static_cast<size_t>(step) < prefix.size() && 
        ...
        ops::Log()(_spare_beam.to(DataType::FLOAT), _spare_float_beam);
        log_prob_beam.copy_from(_spare_float_beam.to(log_prob_beam.dtype()));
      } else {
