...
TEST(TranslatorTest, DidiShortForm) {
  Translator translator("/home/yiqihuang/projects/didi-translate/data/ctranslate2_models/zhen-v4/ctranslate2");
  TranslationOptions options;
  options.prefix_bias_beta = 0.01;
  options.beam_size = 2;
  const std::vector<std::vector<std::string>> input = {
  ...
  auto result = translator.translate_batch_with_prefix(input, prefix, options);
  EXPECT_EQ(result[0].output(), (std::vector<std::string>{"Los", "Angeles"}));

  options.prefix_bias_beta = 0.99;
  result = translator.translate_batch_with_prefix(input, prefix, options);
  EXPECT_EQ(result[0].output(), (std::vector<std::string>{"San", "Francisco"}));
} 

TEST(TranslatorTest, DidiLongForm) {
  Translator translator("/home/yiqihuang/projects/didi-translate/data/ctranslate2_models/zhen-v4/ctranslate2");
  TranslationOptions options;
  const std::vector<std::vector<std::string>> input = {
    {"我们", "是", "滴滴", "洛杉矶", "自然@@", "语言", "处理", "实验室"},
  };
  const std::vector<std::vector<std::string>> prefix = {
    {"We", "are", "the", "Di@@", "Di", "San", "Francisco"},
  };
  options.beam_size = 2;
  options.prefix_bias_beta = 0.01;
  auto result = translator.translate_batch_with_prefix(input, prefix, options);
  EXPECT_EQ(result[0].output(), (std::vector<std::string>{
      "We", "are", "the", "Di@@", "Di", "Los", "Angeles", 
      "Natural", "Language", "Proc@@", "essing", "L@@", "abor@@", "atory", "."}));
